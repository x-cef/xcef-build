...
release dropdown and five searchable multi-select lists: Windows-x64, macOS-arm64,
macOS-x64, Linux-x64, and Linux-arm64. One request can include different targets and versions.
...
builds start. The configured target pairs are Windows/x64, Linux/x64, macOS/arm64, macOS/x64, and Linux/arm64.
...
matrix: CEF `151.3.24+g2384915+chromium-151.0.7922.174` across all five targets.
...
Linux/arm64 uses the native `ubuntu-24.04-arm` runner and the `linuxarm64` CEF SDK.
